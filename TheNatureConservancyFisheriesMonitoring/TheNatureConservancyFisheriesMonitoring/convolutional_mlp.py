...
from logistic_sgd import LogisticRegression, load_data, writePredictionsToCsv, newShape, classLabels, get_feature_test_points
...
def evaluate_lenet5(learning_rate=0.1, n_epochs=200,
    ...
    print(newShape)
    layer0_input = x.reshape((batch_size, 1, newShape[0], newShape[1]))
    ...
    layer0 = LeNetConvPoolLayer(
        rng,
        input=layer0_input,
        image_shape=(batch_size, 1, newShape[0], newShape[1]),
        filter_shape=(nkerns[0], 1, 5, 5),
        poolsize=(2, 2)
    )
    ...
    layer1 = LeNetConvPoolLayer(
        rng,
        input=layer0.output,
        image_shape=(batch_size, nkerns[0], (newShape[0]-5+1)/2, (newShape[1]-5+1)/2),
    ...
    layer3 = LogisticRegression(input=layer2.output, n_in=500, n_out=len(classLabels))
    ...
    num_test_pnts = test_set_x.shape[0]
    test_set_x = test_set_x.reshape((num_test_pnts, 1, newShape[0], newShape[1]))
...
if __name__ == '__main__':
    ...
    if Stage==ClassifierStage.Train or Stage==ClassifierStage.TrainTest : evaluate_lenet5(dataset=Data_Dir)
